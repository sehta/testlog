...
var LuckyLunchSchema = require('./models/luckylunch');
...
var LuckyLunch = mongoose.model('LuckyLunch', LuckyLunchSchema);
...
app.use(function (req, res, next) {
    ...
    //    stream.write("==============Request Date: " + new Date() + "============\r\n");
    //    stream.write("URL:" + url+"\r\n");
    //    stream.write("Body:" + req.body.toString()+"\r\n");
    //    stream.write("====================================");
    //    stream.end();
    //});
    next()
...
//
                  ...
                  User.find({ 
                                 "_id": ObjectId(req.body._id)
                                  ...
                                  if (err || users.length==0 ) console.log("Error(1440):", err);
                                          ...
                                          user.isSendNotification = req.body.isSendNotification;
                                          user.isVocationPeriod = req.body.isVocationPeriod;
                                          user.cardColor = req.body.cardColor;
                                          if (req.body.isDaysUpdated) {
                                              user.days = req.body.days;
                                          }
                                          if (req.body.isVocationPeriod) {
                                              user.vocationFrom = req.body.vocationFrom;
                                              user.vocationTo = req.body.vocationTo;
                                          }
                                          else {
                                              user.vocationFrom ="";
                                              user.vocationTo = "";
  ...
  app.post('/api/updateCuisine', function (req, res) {
      if (req.isAuthenticated()) {
  ...
  app.get('/api/blockUserbyid', function (req, res) {
      ...
      if (ObjectId.isValid(userID)) {
          User.find({ _id: ObjectId(userID) }, function (err, user) {
              ...
              if (err || user.length == 0) console.log("Error(1541):", err);
                  ...
                  var user = user[0];
                  User.find({ _id: ObjectId(blockedID) }, function (err, user2) {
  ...
  app.get('/api/knownUserbyid', function (req, res) {

      // api / blockUser?user=objectid&known=email
      var qs = querystring.parse(req.url.split("?")[1]),
        userID = qs.user;
      knownID = qs.known;
      ...
      if (ObjectId.isValid(userID)) {
          User.find({ _id: ObjectId(userID) }, function (err, user) {
              ...
              if (err || user.length == 0) console.log("Error(1541):", err);
                  ...
                  var user = user[0];
                  User.find({ _id: ObjectId(knownID) }, function (err, user2) {
                      ...
                      if (err || user2.length == 0) {
                          ...
                          var user2 = user2[0];
                          if (user.known.indexOf(user2._id) == -1) {

                              user.known.push(user2._id);

                              var blockedId = user.blocked.indexOf(user2._id);
                              if (blockedId > -1) {
                                  user.blocked.splice(blockedId, 1);
                                  console.log("User was blocked but has now been known");
                              }

                              //res.send(user.name + ", " + user2.name+ "has been blocked.")
                              user.save();
                              res.status(200).send('<h1>You have added ' + user2.name + ' from lunching with you again.</h1>')
                              //res.status(200).send(user.name + ", " + user2.name+ " has been blocked.");
                          }
                          else
                              res.status(200).send('<h1>This user was already knwon!</h1>')
                          //res.status(200).send(user.name+ ", "+ user2.name+ " was already blocked.");
                          //res.send(user.name+ ", "+ user2.name+ "was already blocked.");                         
                      }
...
// Get arranged lunches on specific date
  app.post('/api/todayMatches', function (req, res) {
      var criteria = {
          participants: { $exists: true, $ne: [] },
          date: { "$gte": req.body.startdate, "$lte": req.body.startdate }
      }
  ...
  //    }
  //    LuckyLunch.find(criteria1, function (err, lunches) { 
  //        //if(lunches.length>0)
  //        //{
  //        //    var criteria = {
  //        //        participants: { $exists: true, $ne: [] },
  ...
  //        //}
  //        var len = lunches.length; 
  //        for (var i = 0; i < len; i++) {
  //            (function (i) {
  //                var criteria2 = {
  //                    participants: { $exists: true, $ne: [] },
  //                    companyid: ObjectId(lunches[i].companyid)
  //                }
  //                Match.findOne(criteria2, function (err, singleMatch) {
  ...
  app.get('/api/getluckylunches', function (req, res) {
      getData(req.query.id,function(err, result){
          if(err)
              res.json(err);
          else
              res.json(result);
...
//      var allLunches = [];
//      var criteria = {
//          "luckyMatch": { "$exists": true, "$ne": {} },
//          "companyid": ObjectId(req.query.id)
//      }

//      var cursor = LuckyLunch.find(criteria).cursor();
//      var matchcursor = LuckyLunch.find(criteria).cursor();
//      var usercursor = LuckyLunch.find(criteria).cursor();
//      var testLunches = [];
//      var lcu = LuckyLunch.find(criteria).forEach(function (myDoc) { testLunches.push(myDoc); });
//      console.log(lcu);

//      cursor.on('data', function (doc) {
//          console.log('--Called when start--');
//          allLunches.push(doc);
...
//      //LuckyLunch.find(criteria, function (err, lunches) {
//      //    if (err) {
//      //        console.log(err);
//      //        return;
//      //    }
...
//      //            var criteria2 = {
//      //                participants: { $exists: true, $ne: [] },
//      //                _id: ObjectId(allLunches[i].luckyMatch)
//      //            }
//      //            console.log("------------lunches[i].luckyMatch-------------");
//      //            console.log(allLunches[i].luckyMatch);
//      //            Match.findOne(criteria2, function (err, singleMatch) {
//      //                if (singleMatch != null) {
//      //                    console.log("------------singleMatch.participants-------------");
//      //                    console.log(singleMatch.participants);
//      //                    var userids = [];
...
//      //                        _id: { $in: userids }
//      //                    }
//      //                    User.find(criteria, function (err, getusers) {
...
//      //                        allLunches[i].luckyUsers = usernames;

//      //                        if (i == allLunches.length - 1)
//      //                            res.json(allLunches);
//      //                    });
//      //                }
//      //            });
...
// Save New Lucky lunch Info
  app.post('/api/addLuckyLunch', function (req, res) {
      var luckylunch = req.body;
      var numberlunch = req.body.availableLunch;
      console.log(luckylunch);
      for (var i = 0; i < numberlunch; i++) {
          luckylunch.availableLunch = 1;
          luckylunch.luckyUsers = [];
          addToDatabase(LuckyLunch, luckylunch, "LuckyLunch", null);
      }
      res.send("Admin Request Approved: Added new Lucky Lunch");
  });
...
//});
      ...
      Match.findOne({
          participants: { $in: userids }
                     ...
                     .exec(function (err, result) {
                         if (result) {
                             Match.find({ participants: { $in: userids } }, function (err, glmatches) {
                                 res.json({ 'lastlunch': result.date, 'lunchCount': glmatches.length });
                             });
                         }
                         else
                             res.json({ 'lastlunch': '', 'lunchCount': 0 });

                     });
  });

  //app.get('/api/companylunches', function (req, res) {
  //    var companyids = [];
  //    Match.find({
  //        "companyid": ObjectId(req.query.id)
  //    },function (err, result) {
  //                       if (result)
  //                           res.json({ 'totalLunch': result.length });
  //                       else
  //                           res.json({ 'lastlunch': '' });
  ...
  app.post('/api/companylunches', function (req, res) {
      console.log('-------companylunches----------');
      console.log(req.query.id);
      console.log(req.body.startdate);
      console.log(req.body.enddate);
      var criteria2 = {
          "companyid": ObjectId(req.query.id),
          date: { "$gte": req.body.prevstartdate, "$lt": req.body.prevenddate }
      }
      var criteria = {
          "companyid": ObjectId(req.query.id),
          date: { "$gte": req.body.startdate, "$lt": req.body.enddate }
      ...
      Match.find(criteria, function (err, glmatches) {
          ...
          Match.find({
              ...
              Match.find(criteria2, function (err, prevmatches) {
                  res.json({ 'totalLunch': result.length, 'currentweeklunch': glmatches.length, 'prevweeklunch': prevmatches.length });
              });
          });
      });
  });
            ...
            User.find({
              ...
              blocked : { $ne: ObjectId(d_user._id) } 
                ...
                if( !err && users != undefined && users.length > 0){
                    ...
                    match.companyid = users[0].companyid;  // add companyid to match collection
                  match.save();
